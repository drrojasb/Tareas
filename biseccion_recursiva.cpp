...
template <class fptr>
double bisection_recursiva (double xl, double xu, fptr fun, double eps); 
...
int main (int argc, char **argv)
{
  ...
  std::cout << bisection(XL, XU, f, EPS) << "\t" << bisection_recursiva(XL, XU, f, EPS) << std::endl;
...
{   
    ...
    if (std::fabs(fun(xr)) <= eps){ 
        ...
        if (fun(xr)*fun(xl) < 0){
            return bisection_recursiva(xl, xr, fun, eps);
        }
        else{
            return bisection_recursiva(xr, xu, fun, eps);
        }
    }            
}